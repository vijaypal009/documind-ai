...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    console.log(`Document "${req.file.originalname}" processed (${chunks.length} chunks).`);
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // Retrieve the most relevant chunks or provide full context if small document
    const topChunks = await store.search(question, 5);
